...
class TestDataUpdateTable extends _i1.UpdateTable<TestDataTable> {
  TestDataUpdateTable(super.table);
...
class TestDataTable extends _i1.Table<_i1.UuidValue?> {
  TestDataTable({super.tableRelation}) : super(tableName: 'test_data') {
    updateTable = TestDataUpdateTable(this);
  ...
  late final TestDataUpdateTable updateTable;
...
class TestDataRepository {
  ...
  /// Updates a single [TestData] by its [id] with the specified [columnValues].
  /// Returns the updated row or null if no row with the given id exists.
  Future<TestData?> updateById(
    _i1.Session session,
    _i1.UuidValue id, {
    required _i1.ColumnValueListBuilder<TestDataUpdateTable> columnValues,
    _i1.Transaction? transaction,
  }) async {
    return session.db.updateById<TestData>(
      id,
      columnValues: columnValues(TestData.t.updateTable),
      transaction: transaction,
    );
  }

  /// Updates all [TestData]s matching the [where] expression with the specified [columnValues].
  /// Returns the list of updated rows.
  Future<List<TestData>> updateWhere(
    _i1.Session session, {
    required _i1.ColumnValueListBuilder<TestDataUpdateTable> columnValues,
    ...
    return session.db.updateWhere<TestData>(
      columnValues: columnValues(TestData.t.updateTable),
